@if ($crud->hasAccess('update'))
    @if (!$crud->model->translationEnabled())
        ...
        <!-- Single edit button -->
        <a href="{{ url($crud->route.'/'.$entry->getKey().'/edit') }}" class="btn btn-sm btn-info"><i class="la la-edit"></i> {{ trans('backpack::crud.edit') }}</a>

    @else

        <!-- Edit button group -->
        <div class="btn-group">
            <a href="{{ url($crud->route.'/'.$entry->getKey().'/edit') }}" class="btn btn-sm btn-info pr-0"><i class="la la-edit"></i> {{ trans('backpack::crud.edit') }}</a>
